...
namespace WHAT_API.API_Tests.Lessons
{
    ...
    public class PostAssingingMentorToLesson : API_BaseTest
    {
        ...
        public void AssingingMentorToLesson(HttpStatusCode expectedStatusCode, Role role, int mentorId, int lessonId)
        {
            ...
            AssingingMentorRequest assingingMentorRequest = new AssingingMentorRequest().WithMentorId(mentorId).WithLessonId(lessonId);
            ...
            var resposneDetaile = JsonConvert.DeserializeObject<ResponseAssingingMentor>(response.Content);
            Assert.Multiple(() =>
            {
